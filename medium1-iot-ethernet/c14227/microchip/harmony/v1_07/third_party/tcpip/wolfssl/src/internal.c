        ...
        if (ret == 0) {
            ...
            if (usingEcc) {
                ...
                if (doUserRsa) {
                    ...
                    uint32_t timeTest = SYS_TMR_TickCountGet();
                    ret = wc_RsaSSL_Sign(signBuffer, signSz, verify + extraSz +
                                  VERIFY_HEADER, ENCRYPT_LEN, &key, ssl->rng);
                    uint32_t timeTestafter = SYS_TMR_TickCountGet();
                    SYS_CONSOLE_PRINT("Tick Before: %d, Tick After: %d, Tick Diff: %d\r\n", timeTest, timeTestafter, timeTestafter-timeTest);
                }
